...
var API = 'http://localhost/router/api/admin-dashboard/course/update/';
var APIImageChange = 'http://localhost/router/api/admin-dashboard/course/post/image-change';
var formUpdate = document.getElementById('formUpdate');
var addInRouteChange = document.getElementById('formUpdate').addInRouteChange;
...
var levelCourseUpdate = document.getElementById('formUpdate').levelCourseUpdate;
...
var updateNotImageFunc = function updateNotImageFunc(buttonUpdate) {
  ...
  var validateEmptys = function validateEmptys() {
    if (_.isEmpty(addInRouteChange.value)) return showError('Không được để trống tên lộ trình'); else if (_.isEmpty(courseNameUpdate.value)) return showError('Không được để trống phần tên khóa học'); else if (_.isEmpty(descriptionUpdate.value)) return showError('Không được để trống phần tiêu đề'); else if (_.isEmpty(levelCourseUpdate.value)) return showError('Không được để trống phân cấp độ khóa học'); else if (_.isEmpty(totalTimeUpdate.value)) return showError('Không được để trống tổng thời gian học');
    // else if (_.isEmpty(imageUpdate.value)) return showError("Không được để trống Image");
    if (addInRouteChange.value && courseNameUpdate.value && descriptionUpdate.value && levelCourseUpdate.value && totalTimeUpdate.value) {
      var addInrouteStudyChunkArray = addInRouteChange.value.split(':');
...
var updateHasImageFunc = function updateHasImageFunc(container, stepUpdateImage, imageUpdate) {
  ...
  var validateEmptys = function validateEmptys() {
    if (_.isEmpty(addInRouteChange.value)) return showError('Không được để trống tên lộ trình'); else if (_.isEmpty(courseNameUpdate.value)) return showError('Không được để trống phần tên khóa học'); else if (_.isEmpty(descriptionUpdate.value)) return showError('Không được để trống phần tiêu đề'); else if (_.isEmpty(levelCourseUpdate.value)) return showError('Không được để trống phân cấp độ khóa học'); else if (_.isEmpty(totalTimeUpdate.value)) return showError('Không được để trống tổng thời gian học'); else if (_.isEmpty(imageUpdate.value)) return showError('Không được để trống Image');
    if (addInRouteChange.value && courseNameUpdate.value && descriptionUpdate.value && levelCourseUpdate.value && totalTimeUpdate.value) {
      var addInrouteStudyChunkArray = addInRouteChange.value.split(':');
      formDataUpdate.append('addInRouteChange', addInrouteStudyChunkArray[1]);
      formDataUpdate.append('courseNameUpdate', courseNameUpdate.value);
